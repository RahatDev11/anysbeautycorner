...
import { getDatabase, ref, get, set, query, orderByChild, equalTo, runTransaction, onValue } from "https://www.gstatic.com/firebasejs/10.13.2/firebase-database.js";
...
// =================================================================
// SECTION: AUTHENTICATION & UI CONTROLS
// =================================================================
    ...
    const placeOrder = async (event) => {
        ...
        try {
            ...
            const orderData = {
                ...
                paymentNumber: isOutside ? document.getElementById('paymentNumber').value : 'N/A',
                transactionId: isOutside ? document.getElementById('transactionId').value : 'N/A',
            };
...
async function initializePage() {
    ...
    if (currentPage.endsWith('/') || currentPage.endsWith('index.html')) {
        ...
        await initializeOrderFormPage(); // This will now work correctly
    }
    // `order-track.html` is self-contained and doesn't need to be called from here.
    ...
    try {
        ...
        await new Promise(resolve => {
            onAuthStateChanged(auth, async (user) => {
                ...
                await loadCart(); // Load cart after user state is known
                resolve();
            });
        });
...
document.addEventListener("click", (event) => {
    ...
    if (desktopSubMenu && !desktopSubMenu.contains(event.target) && !desktopSubMenu.closest('[onclick*="toggleSubMenuDesktop"]')) desktopSubMenu.classList.add('hidden');
...
// Assign necessary functions to window object
Object.assign(window, {
    showToast, sendTelegramNotification,
    openSidebar, closeSidebar, toggleSubMenuMobile, handleSubMenuItemClick, toggleSubMenuDesktop,
    openCartSidebar, closeCartSidebar,
    loginWithGmail, confirmLogout,
    addToCart, updateQuantity, removeFromCart, checkout, buyNow,
    showProductDetail, filterProducts,
    changeDetailQuantity, addToCartWithQuantity, buyNowWithQuantity,
    openImageModal, closeImageModal, changeModalImage,
    toggleSocialIcons
});
